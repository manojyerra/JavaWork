...
class Util
{
	...
	static ArrayList<String> getModifiedFilesList(String gitFolder) throws Exception
	{
		String command = "git -C "+gitFolder+" status -uno";
	...
	static ArrayList<DiffBlock> getDiffBlocks(ArrayList<String> diffData)
	{
		ArrayList<DiffBlock> listOfBlocks = new ArrayList<DiffBlock>();

		int startIndex = 0;
		
		for(int i=1; i<diffData.size(); i++)
		{
			char ch0 = diffData.get(i-1).charAt(0);
			char ch1 = diffData.get(i).charAt(0);
			
			boolean isBlockEnd = (ch0 == ' ' && ch1 != ' ') || (ch0 != ' ' && ch1 == ' ');
			
			if(isBlockEnd)
			{
				DiffBlock diffBlock = new DiffBlock(diffData, startIndex, i);
				listOfBlocks.add(diffBlock);
				startIndex = i;
			}
		}
		
		if(startIndex < diffData.size())
		{
			DiffBlock diffBlock = new DiffBlock(diffData, startIndex, diffData.size());
			listOfBlocks.add(diffBlock);
		}
		
		//for(int i=0; i<listOfBlocks.size();i++)
		//	listOfBlocks.get(i).print();
