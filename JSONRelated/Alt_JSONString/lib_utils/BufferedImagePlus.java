...
public class BufferedImagePlus implements Cloneable
{
	...
	{
		Graphics2D graph = imageInfo.texture.createGraphics();
        graph.setColor(color);
        graph.fill(new Rectangle(x, y, w, h));
        graph.dispose();
		
		//imageInfo.texture.getGraphics().setColor(Color.red);
		//imageInfo.texture.getGraphics().fillRect(x, y, w, h);
	}
	...
	public void DrawString(String str, int x, int y, Color color, Font font, boolean wrap)
	{
		...
		else
		{
			...
			String strArr[] = Utils_String.wrap(str, fontMat, GetWidth()-x);
			
			float cellH = font.getSize()+font.getSize()/3;
			
			for(int i=0;i<strArr.length;i++)
				g.drawString(strArr[i], x,(int)(y+(i*cellH)));
		}
	}
	...
	public void SetCanvasSize(int width, int height) throws Exception
	{
		...
		localTexture.getGraphics().drawImage(this.imageInfo.texture, 0,0,textureWidth, textureHeight, null);
