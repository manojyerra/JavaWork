...
class FilesBatchCopy
{
	...
	{
		listName = args[0];
		new FilesBatchCopy(args[0], args[1], args[2]);
	}
	

	FilesBatchCopy(String listFilePath, String srcPath, String destinPath) throws Exception
	{
		...
		try{
			while(isEndOfAddingFiles == false || index < list.size())
			{
				if(index < list.size())
				{
					...
					index++;
					FilesBatchCopy.totalCopied++;
					
					if(FilesBatchCopy.totalCopied % 5000 == 0)
						System.out.println(FilesBatchCopy.listName+" : copied "+FilesBatchCopy.totalCopied);
				}
			}
		}
		catch(Exception e)
