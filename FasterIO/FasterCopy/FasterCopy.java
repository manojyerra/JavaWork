...
class FasterCopy
{
	...
	{
		int numJVMs = 4;
		File src = new File("/home/vagrant/git/comChanges/prpc-platform");
		File dest = new File("/home/vagrant/git/deleteThis");
		...
		if(args.length == 3)
			numJVMs = Integer.parseInt(args[2]);

		new FasterCopy(src, dest, numJVMs);
	}
	
	FasterCopy(File src, File dest, int numJVMs) throws Exception
	{
		System.out.println("Creating folder structure...");
		
		long startTime = System.currentTimeMillis();

		ArrayList<String> list = Folder.createFolderStructure(src, dest);
