...
class LinkedArray<T>
{
	...
	public Vector<Vector<T>> chunkVector = new Vector<Vector<T>>();
	private Vector<T> lastVec;
	private int size; 
	...
	LinkedArray()
	{
		...
		Vector<T> vec = new Vector<T>(CHUNK_LOWER_LIMIT, CHUNK_INCREMENT);
		chunkVector.add(vec);
		lastVec = vec;
		size = 0;
	}
	...
	{
		Vector<T> vec = chunkVector.get(0);
		vec.add(0, element);
		size++;
	...
	public synchronized void remove(int index)
	{
		...
		Vector<T> vec = chunkVector.get(retArr[0]);
	...
	public synchronized int capacity()
	{
		...
		Iterator<Vector<T>> iterator = chunkVector.iterator();
	...
	private void CreateChunkAt(int index, Vector<T> prevChunk)
	{
		Vector<T> newVec = new Vector<T>(CHUNK_LOWER_LIMIT, CHUNK_INCREMENT);						
	...
	private void AddOneToChunkSizes(int fromChunkIndex)
	{
		if(fromChunkIndex > 0)
		{
			...
			for(int i=fromChunkIndex-1; i<s; i++)
				arr[i]++;
		}	
	}
